...
from .convolutional_block import build_DwConv_Block, build_DwConv_Bigkernel_Block
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.conv_proj_q = self._build_projection(
            dim_in=dim, dim_out=dim, kernel_size=3, padding=1,
            stride=1, method='linear' if self.method == 'avg' else self.method
        )
        self.conv_proj_k = self._build_projection(
            dim_in=dim, dim_out=dim, kernel_size=3, padding=1,
            stride=1, method=self.method
        )
    ...
    def forward(self, x, mask=None):
        ...
        if (
            ...
            q, k, v = self.forward_windows_partition(self.forward_conv(x, H, W), C=C)
    ...
    def forward_windows_partition(self, qkv, C=96):
        if isinstance(qkv, (list, tuple)):
            q, k, v = qkv
        else:
            raise TypeError('qkv type error')

        # partition windows
        q = window_partition(q, self.window_size[0])  # nW*B, window_size, window_size, C
        q = q.view(-1, self.window_size[0] * self.window_size[0], C)  # nW*B, window_size*window_size, C

        k = window_partition(k, self.window_size[0])  
        k = k.view(-1, self.window_size[0] * self.window_size[0], C)  

        v = window_partition(v, self.window_size[0])  
        v = v.view(-1, self.window_size[0] * self.window_size[0], C)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
        ...
        # self.dwconv_block_template = build_DwConv_Block(in_planes=dim, out_planes=dim)
        # self.dwconv_block_search = build_DwConv_Block(in_planes=dim, out_planes=dim)
    ...
    def forward(self, x, mask_matrix=None):
        ...
        #     x = self.dwconv_block_search(x)
        #     x = x.permute(0, 2, 3, 1).flatten(1, 2)
        
        # if H == W == 128:
        #     x = x.reshape(B, H, W, C).permute(0, 3, 1, 2)
        #     x = self.dwconv_block_template(x)
        #     x = x.permute(0, 2, 3, 1).flatten(1, 2)
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(shifted_x, mask=attn_mask)  # nW*B, window_size*window_size, C
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim, i_layer, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x, H, W):
        ...
        if self.i_layer < 3:
            ...
            x = torch.cat([x, x, x, x], -1)  # B H W 4*C
            x = x.view(B, -1, 4 * C)  # B H*W 4*C
...
def _build_stages(num_layers,
    ...
    for i_layer in range(num_layers):
        if i_layer == 0:
            ...
            pre_stage = PatchMerging(dim, i_layer, norm_layer)
...
class SwinTransformer_cvt_clean(nn.Module):
    def __init__(self,
                 pretrain_img_size=224,
                 patch_size=4,
                 in_chans=3,
                 ...
                 frozen_stages=-1,
                 use_checkpoint=False):
        super(SwinTransformer_cvt_clean, self).__init__()
    ...
    def forward(self, x, out_indices=None, reshape=True):
        ...
        for i in range(self.num_stages):
            ...
            if i in out_indices:
                ...
                outs['stage' + str(i)] = x_out
        return outs

    def train(self, mode=True):
        """Convert the model into training mode while keep layers freezed."""
        super(SwinTransformer_cvt_clean, self).train(mode)
        _freeze_stages(self.stages, self.frozen_stages, self.ape)
...
_cfg = {
    ...
    'swin_tiny_patch4_window8_224': dict(
        params=dict(pretrain_img_size=224, patch_size=4, window_size=8, embed_dim=96, depths=(2, 2, 6, 4),
                    num_heads=(3, 6, 12, 24))
    ),
}


def build_swin_transformer_backbone(name, load_pretrained=True, output_layers=(2,), frozen_stages=-1,
    ...
    if overwrite_embed_dim is not None:
        ...
        pretrained_model_path = None
    transformer = SwinTransformer_cvt_clean(out_indices=output_layers, frozen_stages=frozen_stages, **params)
