...
from ltr.models.backbone.swin_transt_backbone import build_swin_transformer_cvt_backbone, build_swin_transformer_cvt_clean_backbone
...
from ltr.models.neck.encoder_featurefusion_network import build_encoder_featurefusion_network, build_dwconv_encoder_featurefusion_network
from ltr.models.neck.backbone_enhance import build_backbone_enhance_network
...
class SwinTransT(nn.Module):
    ...
    def __init__(self, backbone, backbone_net_enhance, featurefusion_network, num_classes):
        ...
        self.backbone_enhance = backbone_net_enhance
    ...
    def forward(self, search, templates):
        ...
        src_templates = self.backbone_enhance(src_templates, mask_templates, pos_templates[-1])
        ...
        _, hs = self.featurefusion_network(src_templates, mask_templates,
                                        src_search, mask_search, pos_templates, pos_search[-1])
    ...
    def track(self, search, templates: list):
        ...
        # _, hs = self.featurefusion_network(src_templates, mask_templates, pos_templates,
        #                                       src_search, mask_search, pos_search[-1])
        src_templates = self.backbone_enhance(src_templates, mask_templates, pos_templates)
...
from ltr.models.backbone.swin_transformer_qkvcnn import _cfg
import copy
@model_constructor
def transt_swintranst(settings):
    num_classes = 1
    # name = 'swin_tiny_patch4_window8_224'
    name = settings.swintranst_name
    ...
    backbone_net_enhance = build_backbone_enhance_network(params, settings, output_layer, need_dwconv=True)
