...
import ltr.models.tracking.swin_transt_qkvcnn_encoderfusion as swintranst_models
...
def run(settings):
    ...
    # SwinTransT
    settings.swintranst_name = 'swin_tiny_patch4_window8_224'

    # Transformer
    settings.position_embedding = 'sine'
    settings.hidden_dim = 384
    ...
    settings.env.workspace_dir = '/home/xlsun/xlsun/code/TransT-M/results/SwinTransT_Cvt_iouhead'
    ...
    settings.transt_path = '/home/xlsun/xlsun/code/TransT-M/results/SwinTransT-M/SwinTransT_ep0340.pth.tar'
    ...
    dataset_train = sampler.TransTMaskSampler([got10k_train], [1],
                                            num_search_frames=1, num_template_frames=2,
                                            samples_per_epoch=400*settings.batch_size, max_gap=100, processing=data_processing_train,
                                            frame_sample_mode='transt')
    ...
    model = swintranst_models.transt_swintranst(settings)

    # Wrap the network for multi GPU training
    if settings.multi_gpu:
        model = MultiGPU(model, dim=0)

    objective = swintranst_models.transt_loss(settings)
    ...
    trainer.train(90, transt_path=settings.transt_path, load_latest=False, fail_safe=False)
